...
class DefaultControllerTest extends WebTestCase
{
    public function testHomePage() 
    {
        ...
        $this->assertCount(1, $crawler->filter('h1'), "Le titre de la page est manquant");
        $this->assertCount(1, $crawler->filter('body > nav'), "La nav de la page est manquant");
        $this->assertCount(1, $crawler->filter('body > footer'), "Le footer de la page est manquant");
    ...
    public function testShowListLink()
    {
        ...
        $this->assertSelectorTextContains('html h1', 'Séries');

        // je vérifie que j'ai bien 2 liens vers 2 séries (celles qui sont dans le BDD de test chargée avec les Fixtures)
        // $this->assertCount(100, $crawler->filter(100, '.tv_show_list > a'), "La liste des séries ne contient pas le bon nombre de séries");
